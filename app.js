...
app.ajax = function (type, url, data, callback) {
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === DONE) {
      if (!xhr.responseText) callback(null, xhr);
      else callback(JSON.parse(xhr.responseText), xhr);
    }
  };
};
...
app.render = function (response) {
  if (response.today) {
    ...
    document.querySelector("#today-value").innerHTML =
      "$" + parseFloat(response.today.trm).toLocaleString("us-EN");
    document.querySelector("#today-value2").innerHTML =
      "$" + parseFloat(response.today.trm).toLocaleString("us-EN");
    ...
    document
      .querySelector(".variation")
      .setAttribute(
        "title",
        "Ayer " +
          response.yesterday.dateText +
          ": $" +
          parseFloat(response.yesterday.trm).toLocaleString("us-EN")
      );
...
app.ajax(
  "GET",
  "https://www.dolarcolombia.co/api/get",
  false,
  function (response) {
    app.render(response);
  }
